...
func _toggle(target: Node3D, state: bool) -> void:
	...
	set_physics_process(state); set_process_input(state); ray_query.exclude = []; erase_query.exclude = []
...
func _physics_process(_delta: float) -> void:
	...
	erase_query.from = held.global_position; erase_query.to = held.global_position + (fixed_basis * DIRS[facing] * erase_range)
