...
func _input(event: InputEvent) -> void:
	if not event.is_action_pressed(&"ui_cancel") or animation_player.is_playing(): return
	get_viewport().set_input_as_handled()
	if ui_state == State.OPTIONS and options.has_method(&"on_exit_submenu"): options.on_exit_submenu()

func hide_and_show(first: StringName, second: StringName) -> void:
	animation_player.play(first); await animation_player.animation_finished
